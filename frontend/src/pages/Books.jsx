...
const RentalModal = ({ book, onClose, onRent }) => {
  ...
  // Calculate rental price based on duration (simple calculation for demo)
  ...
  const handlePlaceOrder = (e) => {
    ...
    expiryDate.setDate(today.getDate() + selectedDuration);
    
    // Format dates for display
    const formattedRentedDate = today.toISOString().split('T')[0];
    const formattedExpiryDate = expiryDate.toISOString().split('T')[0];
    
    // Create rental details with all necessary information
    const rentalDetails = {
      id: book.id || Math.random().toString(36).substring(2, 9), // Generate ID if not provided
      title: book.title,
      author: book.author,
      coverImage: book.coverImage || '/api/placeholder/250/350',
      rentedDate: formattedRentedDate,
      expiryDate: formattedExpiryDate,
      rentalDuration: selectedDuration,
      rentalPrice: `$${calculateRentalPrice(book.price, selectedDuration)}`,
      progress: 0, // Starting progress
      ratings: book.ratings
    };
    
    // Pass the rental details to parent component
    onRent(rentalDetails);
    
    // Navigate to my rentals after placing the order
    navigate('/my-rentals');
    onClose();
  };
              ...
              <div className="relative w-full h-full bg-gradient-to-r from-gray-900 to-black">
                ...
                <div className="relative z-20 flex h-full">
                  <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col justify-center">
                    <div className="inline-block px-4 py-1 bg-pink-500/20 text-pink-400 rounded-full text-sm font-medium mb-4">
                      Featured Book
                    </div>
                  ...
                  <div className="hidden md:flex md:w-1/2 items-center justify-center p-8">
                    <div className="w-64 h-80 rounded-lg shadow-2xl transform ro    tate-3 hover:rotate-0 transition-transform duration-300 overflow-hidden">
                      <img
                        src={book.image}
                        alt={book.title}
          ...
          {books.map((book) => (
            ...
            >
              <div className="w-full h-52 bg-gray-800 relative overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-20 w-20 text-pink-400 opacity-30 group-hover:opacity-50 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                  </svg>
                </div>
